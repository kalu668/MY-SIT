...
def portfolio_analytics_api(request):
    """
    JSON API for portfolio analytics charts.
    Returns asset allocation and profit trend data.
    """
    user = request.user
    ...
    for inv in active_investments:
        category = inv.plan.category.replace('_', ' ').title()
        allocation[category] = float(allocation.get(category, 0)) + float(inv.amount)
    
    if not allocation:
        allocation = {'No Investments': 0}

    # 2. Earnings Trend Data (Last 6 Months)
    months_labels = []
    invested_history = []
    profit_history = []
    ...
    for i in range(5, -1, -1):
        # Calculate start/end of the month approximately
        target_date = now - timedelta(days=i*30)
        months_labels.append(target_date.strftime('%b'))
        
        # Get snapshots at that point in time
        snap_invested = Investment.objects.filter(
            user=user, 
            start_date__lte=target_date
        ).aggregate(Sum('amount'))['amount__sum'] or Decimal('0')
        
        snap_profit = Investment.objects.filter(
            user=user,
            start_date__lte=target_date
        ).aggregate(Sum('actual_profit'))['actual_profit__sum'] or Decimal('0')
        
        invested_history.append(float(snap_invested))
        profit_history.append(float(snap_profit))
